...
def menu(
    ...
    while True:
        ...
        if ev.type == MOUSEBUTTONDOWN and ev.button == 1 and r.collidepoint(ev.pos):
            ret = menu[idx].label, idx
            global music_on
            if menu[idx].label == "start game":
            	mode()
            if menu[idx].label == "options":
                configurations()
            if menu[idx].label == "quit":
                game_quit()
            if menu[idx].label == "music":
                change_music()
            if menu[idx].label == "themes":
                change_themes()
            if menu[idx].label == "back":
                main_menu()
            if menu[idx].label == "back to configurations":
                configurations()
            if menu[idx].label == "instructions":
            	instructions()
            if menu[idx].label == "on":
                music_on = True
                main_menu()
            if menu[idx].label == "off":
                music_on = False
                main_menu()
            if menu[idx].label == "v/s computer" or menu[idx].label == "v/s human":
                if menu[idx].label == "v/s computer":
            	    global computer_on
            	    computer_on = True
                countdown()
                parameters_init()
                real_game(menu[idx].label, music_on, computer_on)
                main_menu()
    ...
    return ret
def change_music():
    bg = image.load(join(here,'game.jpg'))
    ...
    while True:
        resp = menu(['on',
                     ...
                     'off',
                       '',
                       ''],
                     font1      = font.Font(join(here,'Edo SZ.ttf'),60),
                     font2      = font.Font(join(here,'Edo SZ.ttf'),60),
                     tooltipfont= font.Font(join(here,"Edo SZ.ttf"),60),
                     color1     = (255,255,255),
                     light      = 9,
                     tooltiptime= 1000,
...
def instructions():
    ...
    #scr.blit(bg,bg.get_rect(center=scr.get_rect().center))
    #display.flip()
    scr.fill((188, 238, 104))
   
    while True:
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("Welcome", smallText, black)
        textRect.center = (650 , 100 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("The following are the controls:", smallText, black)
        textRect.center = (650 , 150 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("Space bar for shooting", smallText, black)
        textRect.center = (650 , 200 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("'w' for choosing weapons", smallText, black)
        textRect.center = (650 , 250 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("Left arrow and Right arrow keys for movement of turret", smallText, black)
        textRect.center = (650 , 300 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("Up and Down keys for changing power", smallText, black)
        textRect.center = (650 , 350 )
        scr.blit(textSurf, textRect)
        smallText = pygame.font.SysFont("Arial",30)
        textSurf, textRect = text_objects("'a' and 'd' for movements of the tanks", smallText, black)
...
def mode():
    ...
    #~ scr.fill(-1)
    display.flip()
    #pygame.mixer.music.load('/home/nikunj/Desktop/pplproject/Master maze - Dreams of above (avalanche theme).m4a')
    #pygame.mixer.music.play(-1)
    while True:
        resp = menu(['v/s computer',
                        '',
                        '',
                     'v/s human',
			'',
...
def countdown():
    scr = display.set_mode((1300,700))
    bg = image.load(join(here,'panzer.png'))
    scr.blit(bg,bg.get_rect(center=scr.get_rect().center))
    if music_on:
        pygame.mixer.music.load('countdown.ogg')
        pygame.mixer.music.play(1)
        

    for count in range(5):
        smallText = pygame.font.SysFont("Arial",300)
        textSurf, textRect = text_objects(str(5 - count), smallText, white)
        textRect.center = (scr.get_width() // 2, scr.get_height() // 2 - bg.get_height() // 5)
        scr.blit(bg,bg.get_rect(center=scr.get_rect().center))
        scr.blit(textSurf, textRect)
        display.flip()
        sleep(1)
if __name__ == '__main__':
    ...
    from pygame import *
    from twotanks2_beautiful import *
    font.init()
    pygame.init()
    ...
    scr = display.set_mode((1300,700))
    c = scr.get_width() + scr.get_height()
    bg = image.load(join(here,'game.jpg'))
    scr.blit(bg,bg.get_rect(center=scr.get_rect().center))
    display.flip()
    while True:
